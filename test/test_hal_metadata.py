...
class HALMetadataTest(unittest.TestCase):
    ...
    def test_metadata_encoding_decoding(self):
        ...
        """
        test_data = [
            (4, [3458764513820540932]),
            (1000, [5764607523034235880]),
            (
                {
        ...
        hal = HardwareAbstractionLayer(
            ...
            HALMetadata(*[i[0] for i in test_data[:4]])
        )

        for metadata_index in range(1, len(test_data) + 2):
            ...
            while True:
                ...
                self.assertEqual(
                    res,
                    (
                        test_data[2][2][output_count][i] if metadata_index == 5
                        else test_data[metadata_index - 1][1][output_count]
                    )
                )
                ...
                if (res >> 61) == metadata_index and (res >> 60) & 1:
                    if metadata_index == 5 and \
                       output_count < len(test_data[2][2]) - 1:
                        output_count += 1
                        i = 0
                        continue
                    else:
