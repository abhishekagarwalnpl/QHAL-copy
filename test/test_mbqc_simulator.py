...
from qhal.quantum_simulators import MBQCQuantumSimulator
from qhal.hal import command_creator, measurement_unpacker


class MockMBQCQuantumSimulator(MBQCQuantumSimulator):

   def get_offset(self, qubit_index: int):
       return self._offset_registers[qubit_index] * 8


class TestMBQCSimulator(unittest.TestCase):
    """
    Test that checks the MBQC output by running a simple test circuit.
    """
    ...
    def test_circuit_equivalence(self):
        ...
        mbqc_backend = MBQCQuantumSimulator(
        ...
        for commands in circuit:
            ...
            mbqc_backend.accept_command(hal_cmd)
    ...
    def test_qubit_index_offset(self):
        ...
        mbqc_backend = MockMBQCQuantumSimulator(
            register_size=10,
            file_name="circuit.txt")
